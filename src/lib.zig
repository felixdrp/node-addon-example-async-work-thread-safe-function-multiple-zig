...
const PRIME_COUNT: u32 = 10000;

const REPORT_EVERY: u32 = 1000;
// Number of calls to addon
const CALLS_MAX_NUMBER: usize = 10;
...
fn StartThread(env: c.napi_env, info: c.napi_callback_info) callconv(.C) c.napi_value {
    ...
    var addon_data_list: *[CALLS_MAX_NUMBER]AddonData = undefined;

    var addon_data_status: usize = 0; // Cero is not set
    var addon_data: *AddonData = undefined;
    var work_num: usize = 0;
    ...
    // Search empty AddonData
    // find one addon_data_status = 1
    // if all workers are busy addon_data_status = 0
    for (addon_data_list.*[0..9], 0..9) |addon_data_element, i| {
        if (addon_data_element.status == 0) {
            work_num = @intCast(i);
            addon_data = &addon_data_list.*[work_num];
            // change state to active or 1
            addon_data.status = 1;
...
fn addon_getting_unloaded(
    ...
    std.heap.raw_c_allocator.free(addon_data_list);
}
